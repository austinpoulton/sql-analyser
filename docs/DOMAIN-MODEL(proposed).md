...
QueriedTable (BaseModel)
    name: str                          # fully-qualified e.g. "schema.table"
...
**What to build:**
1. Create `src/sql_analyser/models.py`
...
4. Implement `QueriedTable` — fields: `name: str`, `columns: list[QueriedColumn] = []`, `has_wildcard: bool = False`
...
8. Create `tests/test_models.py` — test serialisation (round-trip JSON), construction, `canonical_key` symmetry
...
Add merge logic so DataModels from multiple queries can be combined.

**What to build:**
1. Create `src/sql_analyser/merge.py`
...
3. `merge_tables(left: QueriedTable, right: QueriedTable) -> QueriedTable` — assert same `name`; merge column lists by name using `merge_columns`; OR the `has_wildcard` flags
4. `merge_models(left: DataModel, right: DataModel) -> DataModel` — union tables (merge overlapping by name); union relationships (deduplicate via `canonical_key`)
5. Add a convenience `DataModel.merge(other)` method that delegates to `merge_models`
6. Create `tests/test_merge.py` — test: disjoint merge, overlapping tables, type specificity resolution (varchar vs numeric), conflicting non-varchar types (warning), relationship deduplication, wildcard flag propagation
...
**What to build:**
1. Add `ColumnClassification` StrEnum to `models.py`: `MEASURE`, `DIMENSION`, `ATTRIBUTE`
2. Add `OutputColumn` to `models.py` — fields: `alias: str`, `source_table: str | None`, `source_column: str | None`, `transforms: list[str] = []`, `classification: ColumnClassification = ATTRIBUTE`
3. Add `ComplexityMetrics` to `models.py` — fields: `node_count: int`, `scope_count: int`, `scope_types: list[str]`
4. Add `AnalysisResult` to `models.py` — fields: `data_model: DataModel`, `output_columns: list[OutputColumn] = []`, `metrics: ComplexityMetrics | None = None`
5. Add serialisation tests for the new classes in `tests/test_models.py`
...
- `src/sql_analyser/models.py` — all Pydantic domain classes (create new)
- `src/sql_analyser/__init__.py` — public API re-exports (create new)
- `src/sql_analyser/merge.py` — merge logic (create in iteration 2)
...
- `tests/test_models.py` — domain model tests (create new)
- `tests/test_merge.py` — merge logic tests (create in iteration 2)
...
1. **Iteration 1**: `uv run pytest tests/test_models.py` — all core model tests pass (construction, JSON round-trip, canonical_key)
2. **Iteration 2**: `uv run pytest tests/test_merge.py` — all merge scenarios pass (disjoint, overlapping, type resolution, dedup)
3. **Iteration 3**: `uv run pytest tests/test_models.py` — extended tests for OutputColumn, ComplexityMetrics, AnalysisResult serialisation
...
- **No frozen models**: Merge operations create new instances. Pydantic's default structural equality (`__eq__` on all fields) is correct for assertion in tests. Merge matching uses explicit name-based lookup.

## Scope

**Included:** All domain model classes, enums, serialisation, merge logic, and rendering methods.
**Excluded:** Analyser logic (scope traversal, column extraction), lineage computation logic, metrics computation logic — these are downstream consumers of the domain model, built in Phases 2–9 per AGENTS.md.
